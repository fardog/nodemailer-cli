...
var cli = function(options) {
  this.options = {
    ...
    'boolean': ['nossl', 'notls', 'ignorecert', 'version', 'help'],
    'string': ['server', 'service', 'username', 'password', 'subject', 'attachment']
  };
...
cli.prototype.parse = function(argv, next) {
  ...
  if (options.version) {
    ...
    options.useTLS = options.notls ? false : true;
     ...
     'body', 'attachment'].forEach(function(i) {
       if(typeof options[i] !== 'undefined') {
         if (typeof options[i] !== 'string' || options[i].length < 1) {
           //Deal with edge case for attachment which can also be an array
           if (typeof options[i] == 'object' && i == 'attachment') {
